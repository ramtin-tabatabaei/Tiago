...
def detect_markers(frame, aruco_dict, parameters, cameraMatrix, distCoeffs):
    ...
    if ids is not None:
        ...
        for i, (rvec, tvec) in enumerate(zip(rvecs, tvecs)):
            ...
            print(f"Location (x, y, z): ({tvec[0][0]:.4f}, {tvec[0][1]:.4f}, {tvec[0][2]:.4f})")
...
cameraMatrix = np.array([[1432.72847, 0, 919.085362],
                        [0, 1432.48996, 503.954082],
                        [0, 0, 1]] , dtype=float)


#distCoeffs = np.zeros((4, 1))  # Assuming no lens distortion
distCoeffs = np.array([[ 0.04514801 , -0.18810489 , -0.00213181 , -0.00188061 , 0.14742416]], dtype=float)
