...
class ArucoDetector:
    def __init__(self):
        ...
        # self.cameraMatrix = np.array([[fx, 0.0, rx],
        #                               [0.0, fy, ry],
        ...
        # self.distCoeffs = np.array([[0.034373339868345086, -0.12169690916340495,  -0.007093351005146207, 0.002392767261606366, 0]], dtype=float)
        ...
        # self.distCoeffs = np.array([[3.88645697e+00, -7.46598399e+01,  8.81879384e-01, -2.14201274e-02,9.19116000e+03]], dtype=float)
    ...
    def detect_markers(self, frame, aruco_dict, parameters, cameraMatrix, distCoeffs):
        ...
        if ids is not None:
            ...
            for i, corner in enumerate(corners):
                ...
                # quat = rotationVectorToQuaternion(rvecs[i])
                print(f"Marker ID: {ids[i][0]}")
                marker_id = ids[i][0]
                X_avg = int((corners[i][0][0][0]+corners[i][0][1][0])/2)
                Y_avg = int((corners[i][0][0][1]+corners[i][0][2][1])/2)
